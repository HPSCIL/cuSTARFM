...
void Re_fusion2(CuLayer *psensor,PARAMETER *par)
{
	...
	if(par->NUM_PAIRS==1)
	{
	runtest(psensor[0].getData(),psensor[1].getData(),psensor[2].getData(),psensor[3].getData(),psensor[0].getHeight(),psensor[0].getWidth(),par->WIN_SIZE,par->L_ERR,par->M_ERR,par->class_num,par->A);
	//char* driverName = "GTiff";
	psensor[3].Write(par->outpath);
	}
	else
	{
		...
		{
			runtest_pairs(psensor[i].getData(),psensor[i+par->NUM_PAIRS].getData(),psensor[2*par->NUM_PAIRS].getData(),psensor[2*par->NUM_PAIRS+1].getData(),weight,psensor[0].getHeight(),psensor[0].getWidth(),par->WIN_SIZE,par->L_ERR,par->M_ERR,par->class_num,par->A);
			for( m=0;m<psensor[0].getHeight();m++)
			{
			//	j=j;
				for( j=0;j<psensor[0].getWidth();j++)
				{
					single[m*psensor[0].getWidth()+j]+=psensor[2*par->NUM_PAIRS+1].getData()[m*psensor[0].getWidth()+j];
		...
		for( m=0;m<psensor[0].getHeight();m++)
			{
				for( j=0;j<psensor[0].getWidth();j++)
				{
					psensor[2*par->NUM_PAIRS+1].getData()[m*psensor[0].getWidth()+j]=single[m*psensor[0].getWidth()+j]/weight_all[m*psensor[0].getWidth()+j];
				}
			}
		psensor[2*par->NUM_PAIRS+1].Write(par->outpath);
		delete []weight;
		delete []weight_all;
		delete []single;
	}
